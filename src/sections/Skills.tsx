...
import { FiCode, FiLayout, FiSmartphone, FiGitBranch, FiBox, FiZap } from 'react-icons/fi';

const skills = [
  { name: 'JavaScript (ES6+)', icon: <SiJavascript size={28} />, color: '#f7df1e', level: 88 },
  { name: 'TypeScript', icon: <SiTypescript size={28} />, color: '#3178c6', level: 82 },
  { name: 'React.js', icon: <SiReact size={28} />, color: '#009BD4', level: 90 },
  { name: 'Vite', icon: <SiVite size={28} />, color: '#646cff', level: 80 },
  { name: 'Tailwind CSS', icon: <SiTailwindcss size={28} />, color: '#009BD4', level: 85 },
  { name: 'REST APIs', icon: <FiZap size={28} />, color: '#FF8A3D', level: 78 },
  { name: 'Git & GitHub', icon: <SiGithub size={28} />, color: '#475569', level: 85 },
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: '13px', color: '#009BD4', fontWeight: '700' }}>
          {level}%
        </span>
      </div>
      <div style={{ background: '#F1F5F9', borderRadius: '99px', height: '6px', overflow: 'hidden' }}>
        <motion.div
          ...
          style={{
            height: '100%',
            background: `linear-gradient(90deg, ${color}, #00C2FF)`,
            borderRadius: '99px',
          }}
        />
      </div>
